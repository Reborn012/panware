...
async function validateAndSaveScore(storage, payload) {
  const payloadToSave = chunkWAGAASWE_cjs.saveScorePayloadSchema.parse(payload);
...
exports.Mastra = class Mastra {
  ...
  constructor(config) {
    ...
    this.#serverCache = new chunkKTCBGNCZ_cjs.InMemoryServerCache();
    if (config?.pubsub) {
    ...
    const workflowEventProcessor = new chunkJRTFV63C_cjs.WorkflowEventProcessor({
      mastra: this
    });
    ...
    if (config?.observability) {
      chunkGGN63GU5_cjs.setupAITracing(config.observability);
    }
...
`
  ...
  registerAITracingExporters() {
    const allTracingInstances = chunkGGN63GU5_cjs.getAllAITracing();
   ...
   */
  async shutdown() {
    await chunkGGN63GU5_cjs.shutdownAITracingRegistry();
...
//# sourceMappingURL=chunk-UMATHB4Q.cjs.map
//# sourceMappingURL=chunk-UMATHB4Q.cjs.map