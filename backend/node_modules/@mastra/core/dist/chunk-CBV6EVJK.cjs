...
var chunkWECJNCGT_cjs = require('./chunk-WECJNCGT.cjs');
var chunkABJOUEVA_cjs = require('./chunk-ABJOUEVA.cjs');
var chunkTKFLL33F_cjs = require('./chunk-TKFLL33F.cjs');
var chunkTRZT7QPJ_cjs = require('./chunk-TRZT7QPJ.cjs');
...
var chunkGGN63GU5_cjs = require('./chunk-GGN63GU5.cjs');
...
var chunkOVVKYARB_cjs = require('./chunk-OVVKYARB.cjs');
...
var import_fast_deep_equal2=chunkEBVYYC2Q_cjs.__toESM(require_fast_deep_equal(),1);var ProcessorState=class{accumulatedText="";customState={};streamParts=[];constructor(_processorName){}// Internal methods for the runner
...
var ChunkFrom=/* @__PURE__ */(ChunkFrom2=>{ChunkFrom2["AGENT"]="AGENT";ChunkFrom2["USER"]="USER";ChunkFrom2["SYSTEM"]="SYSTEM";ChunkFrom2["WORKFLOW"]="WORKFLOW";return ChunkFrom2;})(ChunkFrom||{});// src/stream/aisdk/v5/transform.ts
...
var MastraModelOutput=class extends chunk4L3P3PCP_cjs.MastraBase{#status="running";#aisdkv5;#error;#baseStream;#bufferedSteps=[];#bufferedReasoningDetails={};#bufferedByStep={text:"",reasoning:"",sources:[],files:[],toolCalls:[],toolResults:[],msgCount:0};#bufferedText=[];#bufferedTextChunks={};#bufferedSources=[];#bufferedReasoning=[];#bufferedFiles=[];#toolCallArgsDeltas={};#toolCallDeltaIdNameMap={};#toolCalls=[];// TODO: add type
...
runtimeContext,onFinish:result=>{streamPromise.resolve(result.text);},abortSignal});stream=fullStream;}else {const modelOutput=await params.streamVNext(inputData.prompt,{runtimeContext,onFinish:result=>{streamPromise.resolve(result.text);},abortSignal});stream=modelOutput.fullStream;}if(streamFormat==="aisdk"){await emitter.emit("watch-v2",{type:"tool-call-streaming-start",...(toolData??{})});for await(const chunk of stream){if(chunk.type==="text-delta"){await emitter.emit("watch-v2",{type:"tool-call-delta",...(toolData??{}),argsTextDelta:chunk.textDelta});}}await emitter.emit("watch-v2",{type:"tool-call-streaming-finish",...(toolData??{})});}else {for await(const chunk of stream){await writer.write(chunk);}}if(abortSignal.aborted){return abort();}return {text:await streamPromise.promise};}};}if(params instanceof chunkOVVKYARB_cjs.Tool){if(!params.inputSchema||!params.outputSchema){throw new Error("Tool must have input and output schemas defined");}return {// TODO: tool probably should have strong id type
// @ts-ignore
...
var languageModelUsageSchema=z5__default.default.object({inputTokens:z5__default.default.number(),outputTokens:z5__default.default.number(),totalTokens:z5__default.default.number(),reasoningTokens:z5__default.default.number().optional(),cachedInputTokens:z5__default.default.number().optional()});var llmIterationStepResultSchema=z5__default.default.object({reason:z5__default.default.string(),warnings:z5__default.default.array(z5__default.default.any()),isContinued:z5__default.default.boolean(),logprobs:z5__default.default.any().optional(),totalUsage:languageModelUsageSchema.optional(),headers:z5__default.default.record(z5__default.default.string()).optional(),messageId:z5__default.default.string().optional(),request:z5__default.default.record(z5__default.default.any()).optional()});var llmIterationOutputSchema=z5__default.default.object({messageId:z5__default.default.string(),messages:z5__default.default.object({all:z5__default.default.array(z5__default.default.any()),// ModelMessage[] but too complex to validate at runtime
user:z5__default.default.array(z5__default.default.any()),nonUser:z5__default.default.array(z5__default.default.any())}),output:z5__default.default.object({text:z5__default.default.string().optional(),reasoning:z5__default.default.array(z5__default.default.any()).optional(),reasoningText:z5__default.default.string().optional(),files:z5__default.default.array(z5__default.default.any()).optional(),// GeneratedFile[]
toolCalls:z5__default.default.array(z5__default.default.any()).optional(),// TypedToolCall[]
toolResults:z5__default.default.array(z5__default.default.any()).optional(),// TypedToolResult[]
sources:z5__default.default.array(z5__default.default.any()).optional(),// LanguageModelV2Source[]
staticToolCalls:z5__default.default.array(z5__default.default.any()).optional(),dynamicToolCalls:z5__default.default.array(z5__default.default.any()).optional(),staticToolResults:z5__default.default.array(z5__default.default.any()).optional(),dynamicToolResults:z5__default.default.array(z5__default.default.any()).optional(),usage:languageModelUsageSchema,steps:z5__default.default.array(z5__default.default.any())// StepResult[]
}),metadata:z5__default.default.object({id:z5__default.default.string().optional(),model:z5__default.default.string().optional(),modelId:z5__default.default.string().optional(),timestamp:z5__default.default.date().optional(),providerMetadata:z5__default.default.record(z5__default.default.any()).optional(),headers:z5__default.default.record(z5__default.default.string()).optional(),request:z5__default.default.record(z5__default.default.any()).optional()}),stepResult:llmIterationStepResultSchema});var toolCallInputSchema=z5__default.default.object({toolCallId:z5__default.default.string(),toolName:z5__default.default.string(),args:z5__default.default.record(z5__default.default.any()),providerMetadata:z5__default.default.record(z5__default.default.any()).optional(),providerExecuted:z5__default.default.boolean().optional(),output:z5__default.default.any().optional()});var toolCallOutputSchema=toolCallInputSchema.extend({result:z5__default.default.any(),error:z5__default.default.any().optional()});// src/loop/workflows/agentic-execution/llm-execution-step.ts
...
function createStreamStep({capabilities,runId,returnScorerData,format="mastra",requireToolApproval,resumeContext}){return createStep({id:"stream-text-step",inputSchema:z5.z.any(),// tried to type this in various ways but it's too complex
...
messages:messageList.get.remembered.v1(),// these are here for inspecting but shouldn't be returned by the processor
// - ex TokenLimiter needs to measure all tokens even though it's only processing remembered messages
newMessages:messageList.get.input.v1(),systemMessage,memorySystemMessage:memorySystemMessage||void 0});const processedList=new chunkL7Z3C5XG_cjs.MessageList({threadId:threadObject.id,resourceId,generateMessageId:this.#mastra?.generateId?.bind(this.#mastra),// @ts-ignore Flag for agent network messages
...
//# sourceMappingURL=chunk-CBV6EVJK.cjs.map
//# sourceMappingURL=chunk-CBV6EVJK.cjs.map